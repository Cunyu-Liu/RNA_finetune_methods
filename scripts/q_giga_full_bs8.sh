#!/bin/bash
# giga-650M full tuned @1e-5（batch 8——OOM 修复：默认 32 需 ~26G+ 挤爆 40G 卡）
# 口径对齐 q_rnasc650_tests.sh 的 650M 协议（bs 8）
...
GPU=$1
LOG=$R/logs/q_giga_full_bs8_g${GPU}.log
...
for SP in random family; do
  for S in 17 29 43; do
    RID=ft_rinalmo650m_noncodingrnafamily_full_s${S}_${SP}_lr1e-05
    ATT=0
    while [ $ATT -lt 3 ]; do
      D=$($PY -c "
import json
...
with open(P,\"w\") as f: fcntl.flock(f,fcntl.LOCK_EX); f.writelines(out)" >> $LOG 2>&1
        ...
        echo "wait GPU$GPU free ${F}G < 24G $(date +%T)" >> $LOG
      fi
      sleep 300
    done
  done
done
echo "GIGA-FULL-BS8 DONE $(date)" >> $LOG
