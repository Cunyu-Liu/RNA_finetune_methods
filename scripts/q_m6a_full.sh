#!/bin/bash
# A8 跨任务补全: <model> <gpu> — m6A full 臂
# 阶段1 default 3e-4 x6 -> 阶段2 判崩(random均值<0.75) -> 阶段3 s101 网格 {1e-5,3e-5} -> tuned x6
...
GPU=$2
TAG=$(echo $MODEL | tr -d "-")
LOG=$R/logs/q_m6a_full_${TAG}_g${GPU}.log
...
for split in random family; do
  for seed in 17 29 43; do
    echo "=== $MODEL m6A full(default) s$seed $split GPU$GPU $(date +%T) ===" >> $LOG
...
for l in open(\"$R/ledger.jsonl\"):
    r = json.loads(l)
    rid = r.get(\"run_id\",\"\")
    if (r.get(\"model\")==\"$MODEL\" and r.get(\"task\")==\"modification\"
...
OK=$($PY -c "print(1 if float(\"$AVG\") >= 0.75 else 0)")
if [ "$OK" = "1" ]; then
  echo "SURVIVOR — skip tuned arm (avg=$AVG)" >> $LOG
  echo "M6A-FULL $MODEL DONE (survivor) $(date)" >> $LOG
  exit 0
fi

for lr in 1e-5 3e-5; do
  echo "=== TUNING $MODEL m6A full s101 lr=$lr GPU$GPU $(date +%T) ===" >> $LOG
  timeout 14400 $PY -m rnafteval.finetune_base --model $MODEL \
    --task modification --strategy full --seed 101 --split random \
...
for split in random family; do
  for seed in 17 29 43; do
    echo "=== FORMAL $MODEL m6A full s$seed $split lr=$BEST GPU$GPU $(date +%T) ===" >> $LOG
    timeout 14400 $PY -m rnafteval.finetune_base --model $MODEL \
      --task modification --strategy full --seed $seed --split $split \
...
echo "M6A-FULL $MODEL DONE BEST=$BEST $(date)" >> $LOG
