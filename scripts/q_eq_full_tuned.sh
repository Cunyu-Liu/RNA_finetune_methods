#!/bin/bash
# 通用全参 tuned 链: <model> <gpu> —— s101 网格 -> formal 双切分 x 3 种子
...
GPU=$2
TAG=$(echo $MODEL | tr -d "-")
LOG=$R/logs/q_eq_fulltuned_${TAG}_g${GPU}.log
...
assert free > 6e9, "free %.2fG" % (free/1e9)
...
echo "EQ-FULLTUNED $MODEL DONE BEST=$BEST $(date)" >> $LOG
