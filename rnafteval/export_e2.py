"""Export E2 PEFT horizontal-comparison table (C5) — dual-model.

Ledger -> markdown + CSV. Five arms × two models (RiNALMo-micro 33M +
RNA-Sc-10M controlled replication, spec v1.4): full (tuned protocol
arm where applicable) / DoRA / LoRA / IA3 / head-only, ncRNA-family
random split, formal seeds 17/29/43.

Usage: python -m rnafteval.export_e2 [--out status/e2_table.md]
"""
from __future__ import annotations

import argparse
import collections
import json
import os

ROOT = "/mnt/cunyuliu/rna-ft-eval"
LEDGER = os.path.join(ROOT, "ledger.jsonl")

ARM_ORDER = ["full", "dora", "lora", "ia3", "headonly"]
ARM_LABEL = {"full": "full FT (LR-tuned 1e-5)", "dora": "DoRA r=8",
             "lora": "LoRA r=8", "ia3": "IA3",
             "headonly": "head-only"}
ARM_PARAMS = {"full": 33482412, "dora": 574560, "lora": 545280,
              "ia3": 11520, "headonly": 0}
MODEL_META = [
    ("rinalmomicro", "RiNALMo-micro (33M)", "33.5M"),
    ("rnasc10m", "RNA-Sc-10M (10M, controlled)", "10M"),
]


def rows_by_arm(model: str) -> dict:
    rows = [json.loads(l) for l in open(LEDGER) if l.strip()]
    out = collections.defaultdict(dict)
    for r in rows:
        if r.get("status") != "done" or r.get("smoke"):
            continue
        rid = r["run_id"]
        if "%s_noncodingrnafamily_" % model not in rid:
            continue
        if r.get("seed") == 101 or "_e3" in rid:
            continue
        if "_random" not in rid:
            continue
        for arm in ARM_ORDER:
            tag = "_%s_" % arm if arm != "headonly" else "_headonly_"
            if tag in rid:
                if "_lr" in rid:
                    # tuned 协议臂: full@1e-05 覆盖（RNA-Sc 无 tuned
                    # 行——默认 LR 即正常, A8 网格 0.688）
                    if arm == "full" and "_lr1e-05" in rid:
                        out[arm][r["seed"]] = float(r["value"])
                else:
                    out[arm].setdefault(r["seed"], float(r["value"]))
    return out


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", default=os.path.join(ROOT, "status",
                                                  "e2_table.md"))
    args = ap.parse_args()
    lines = []
    csv_rows = []
    for model, mlabel, pbase in MODEL_META:
        arms = rows_by_arm(model)
        if not any(arms.get(a) for a in ARM_ORDER):
            continue
        lines += [
            "# E2 PEFT 五臂横评（C5）— %s, ncRNA-family, random" % mlabel,
            "",
            "协议：formal seeds 17/29/43；RiNALMo full = tuned 协议臂"
            "（_lr1e-05 覆盖崩溃行）；RNA-Sc full = 默认 LR（A8 网格"
            "下正常 0.688）；PEFT 臂默认 LR 3e-4。",
            "",
            "| arm | 3-seed mean | seeds | per-seed |"
            " trainable (backbone) |",
            "|---|---|---|---|---|",
        ]
        ranked = sorted(
            ((arm, sum(v.values()) / len(v), v) for arm, v in arms.items()
             if v), key=lambda x: -x[1])
        for arm, mean, seeds in ranked:
            per_seed = ", ".join("s%d %.4f" % (sd, v)
                                 for sd, v in sorted(seeds.items()))
            params = ARM_PARAMS.get(arm)
            p_str = (pbase if params and params > 1e6
                     else ("~%.2fM" % (params / 1e6) if params else "0"))
            lines.append("| %s | %.3f | %d | %s | %s + head |"
                         % (ARM_LABEL.get(arm, arm), mean, len(seeds),
                            per_seed, p_str))
            csv_rows.append((model, arm, mean, len(seeds)))
        lines += ["",
                  "注：DoRA/LoRA 训练参数 ~0.55–0.57M（r=8, qkv+out）；"
                  "IA3 11.5K；prefix-tuning 不可行（peft 0.13 / "
                  "transformers 5.0 Cache API 不兼容，见 preprint "
                  "limitations）。",
                  "数据源：ledger.jsonl（自动导出，export_e2.py）。",
                  ""]
    body = "\n".join(lines) + "\n"
    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    open(args.out, "w").write(body)
    csv_path = os.path.splitext(args.out)[0] + ".csv"
    with open(csv_path, "w") as f:
        f.write("model,arm,mean,n_seeds\n")
        for model, arm, mean, n in csv_rows:
            f.write("%s,%s,%.4f,%d\n" % (model, arm, mean, n))
    print(body)
    print("written:", args.out, "and", csv_path)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
